...
local function require(path)
    ...
    local f = modules[path]
    if not f then
        error("module '" .. tostring(path) .. "' not found in bundle")
    end
    local res = f()
    if res == nil then
        res = true
...
modules["components/KeySystem"] = function()
    local KeySystem = {}
    ...
    function KeySystem.new(Config, Filename, func, keyValidator)
    	local KeyDialogInit = require("components/window/Dialog")
    	local KeyDialog = KeyDialogInit.Create(true, "Popup", Config.Window, Config.CUSTOMGUI, Config.CUSTOMGUI.ScreenGui.KeySystem)
    	...
    	if Config.KeySystem.API then
    		...
    		for _, i in next, Config.KeySystem.API do
    			local serviceDef = Config.CUSTOMGUI.Services[i.Type]
    			if serviceDef then
    				local args = {}
...
modules["components/popup/Element"] = function()
    local Element = {}
...
modules["components/popup/Init"] = function()
    local PopupModule = {}
    ...
    function PopupModule.new(PopupConfig, Parent)
        ...
        local Dialog = DialogInit.Create(true, "Popup", PopupConfig.CUSTOMGUI.Window, PopupConfig.CUSTOMGUI, Parent)
...
modules["components/search/Icons"] = function()
    return { -- lucide
...
modules["components/search/Init"] = function()
    local cloneref = (cloneref or clonereference or function(instance)
    	return instance
    end)
    ...
    function SearchBar.new(TabModule, Parent, OnClose)
    	local SearchBarModule = {
    		...
    		Icons = require("components/search/Icons"),
    	}
...
modules["components/ui/Button"] = function()
    local Button = {}
...
modules["components/ui/Checkbox"] = function()
    local Checkbox = {}
...
modules["components/ui/Code"] = function()
    local Code = {}
...
modules["components/ui/Dropdown"] = function()
    local DropdownMenu = {}
    ...
    function DropdownMenu.New(Config, Dropdown, Element, CanCallback, Type)
    	...
    	Dropdown.UIElements.MenuCanvas = New("Frame", {
    		...
    		Active = false,
    		--GroupTransparency = 1, -- 0
    		Parent = Config.CUSTOMGUI.DropdownGui,
    		AnchorPoint = Vector2.new(1, 0),
    	}, {
    	...
    	local function RecalculateListSize()
    		local MaxHeight = Config.CUSTOMGUI.DropdownGui.AbsoluteSize.Y
...
modules["components/ui/Input"] = function()
    local Input = {}
...
modules["components/ui/Label"] = function()
    local Label = {}
...
modules["components/ui/ScrollSlider"] = function()
    local ScrollSlider = {}
...
modules["components/ui/Tag"] = function()
    local Tag = {}
...
modules["components/ui/Toggle"] = function()
    local Toggle = {}
    	...
    	function Toggle:Animate(input, ToggleObj)
    		if not Config.Window.IsToggleDragging then
    			...
    			endConnection = UserInputService.InputEnded:Connect(function(inputEnded)
    				...
    				Config.CUSTOMGUI.CurrentInput = nil
...
modules["components/ui/Tooltip"] = function()
    local Tooltip = {}
...
modules["components/window/Dialog"] = function()
    local Creator = require("modules/Creator")
    local New = Creator.New
    local Tween = Creator.Tween
    ...
    function DialogModule.Create(Key, Type, Window, CUSTOMGUI, Parent)
    	local Dialog = {
    		UICorner = 28,
    		UIPadding = 12,
...
modules["components/window/Element"] = function()
...
modules["components/window/Init"] = function()
    ...
    --local UIComponents = require("components/Components/UI.lua")
    ...
    return function(Config)
    	...
    	if not RunService:IsStudio() and Window.Folder and writefile then
    		if not isfolder("CUSTOMGUI/" .. Window.Folder) then
    			makefolder("CUSTOMGUI/" .. Window.Folder)
    		end
    		if not isfolder("CUSTOMGUI/" .. Window.Folder .. "/assets") then
    			makefolder("CUSTOMGUI/" .. Window.Folder .. "/assets")
    		end
    	...
    	if typeof(Window.Background) == "string" and BGVideo then
    		...
    		if string.find(BGVideo, "http") then
    			...
    			if not isfile(videoPath) then
    				...
    				if not success then
    					warn("[ CUSTOMGUI.Window.Background ] Failed to download video: " .. tostring(result))
    				end
    			end
    			...
    			if not success then
    				warn("[ CUSTOMGUI.Window.Background ] Failed to load custom asset: " .. tostring(customAsset))
    			end
    			warn("[ CUSTOMGUI.Window.Background ] VideoFrame may not work with custom video")
    			BGVideo = customAsset
    		end
    	...
    	Window.UIElements.Main = New("Frame", {
    		...
    		Config.CUSTOMGUI.UIScaleObj,
    		Window.AcrylicPaint and Window.AcrylicPaint.Frame or nil,
    		Blur,
    		Creator.NewRoundFrame(Window.UICorner, "Squircle", {
    			ImageTransparency = 1, --  Window.Transparent and Config.CUSTOMGUI.TransparencyValue or 0,
    			Size = UDim2.new(1, 0, 1, 0),
    			AnchorPoint = Vector2.new(0.5, 0.5),
    			Position = UDim2.new(0.5, 0, 0.5, 0),
    			Name = "Background",
    	...
    	Creator.AddSignal(Window.UIElements.Main.Main.Topbar.Left:GetPropertyChangedSignal("AbsoluteSize"), function()
    		...
    		local RightWidth = Window.UIElements.Main.Main.Topbar.Right.UIListLayout.AbsoluteContentSize.X
    			/ Config.CUSTOMGUI.UIScale
    
    		LeftWidth = Window.UIElements.Main.Main.Topbar.Left.AbsoluteSize.X / Config.CUSTOMGUI.UIScale
    		...
    		Window.UIElements.Main.Main.Topbar.Center.Position =
    			UDim2.new(0, LeftWidth + (Window.UIPadding / Config.CUSTOMGUI.UIScale), 0.5, 0)
    		Window.UIElements.Main.Main.Topbar.Center.Size = UDim2.new(
    			1,
    			-LeftWidth
    				- (Window.UIPadding / Config.CUSTOMGUI.UIScale)
    	...
    	if Window.Topbar.ButtonsType ~= "Default" then
    		Creator.AddSignal(Window.UIElements.Main.Main.Topbar.Right:GetPropertyChangedSignal("AbsoluteSize"), function()
    			Window.UIElements.Main.Main.Topbar.Left.Position = UDim2.new(
    				0,
    				(Window.UIElements.Main.Main.Topbar.Right.AbsoluteSize.X / Config.CUSTOMGUI.UIScale) + Window.UIPadding - 4,
    				0,
    				0
    			)
    		end)
    	...
    	if not IsVideoBG and Window.Background and typeof(Window.Background) == "table" then
    		...
    		Window.UIElements.BackgroundGradient = Creator.NewRoundFrame(Window.UICorner, "Squircle", {
    			...
    			ImageTransparency = Window.Transparent and Config.CUSTOMGUI.TransparencyValue or 0,
    		}, {
    			BackgroundGradient,
    		})
    	end
    
    	-- local blur = require("components/Blur")
    	...
    	Window.OpenButtonMain = require("components/window/Openbutton").New(Window)
    	...
    	function Window:SetBackgroundTransparency(v)
    		...
    		Config.CUSTOMGUI.TransparencyValue = rounded
    	...
    	local function SetSize(isAnimation)
    		Tween(Window.UIElements.Main, 0.45, {
    			Size = not Window.IsFullscreen and CurrentSize or UDim2.new(
    				0,
    				(Config.CUSTOMGUI.ScreenGui.AbsoluteSize.X - 20) / Config.CUSTOMGUI.UIScale,
    				0,
    				(Config.CUSTOMGUI.ScreenGui.AbsoluteSize.Y - 20 - 52) / Config.CUSTOMGUI.UIScale
    			),
    		}, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
    	...
    	Creator.AddSignal(Config.CUSTOMGUI.ScreenGui:GetPropertyChangedSignal("AbsoluteSize"), function()
    	...
    	if Config.CUSTOMGUI.UseAcrylic then
    	...
    	function Window:Open()
    		...
    		task.spawn(function()
    			...
    			Config.CUSTOMGUI.UIScaleObj.Scale -= 1 - 0.85
    			Tween(
    				Config.CUSTOMGUI.UIScaleObj,
    				0.33,
    				{ Scale = Config.CUSTOMGUI.UIScale },
    			...
    			Config.CUSTOMGUI:ToggleAcrylic(true)
    			--end)
    
    			task.spawn(function()
    				task.wait(0.1)
    				Window:EnableMobileControls()
    			end)
    		end)
    	end
    	function Window:Close()
    		...
    		Config.CUSTOMGUI:ToggleAcrylic(false)
    		...
    		Tween(
    			Config.CUSTOMGUI.UIScaleObj,
    			0.28,
    			{ Scale = Config.CUSTOMGUI.UIScale - (1 - 0.85) },
    			Enum.EasingStyle.Quint,
    			Enum.EasingDirection.Out
    		):Play()
    		if BGImage then
    		...
    		function Close:Destroy()
    			task.spawn(function()
    				...
    				Config.CUSTOMGUI.ScreenGui:Destroy()
    				Config.CUSTOMGUI.NotificationGui:Destroy()
    				Config.CUSTOMGUI.DropdownGui:Destroy()
    				Config.CUSTOMGUI.TooltipGui:Destroy()
    	...
    	function Window:ToggleTransparency(Value)
    		...
    		Config.CUSTOMGUI.Transparent = Value
    
    		Window.UIElements.Main.Background.ImageTransparency = Value and Config.CUSTOMGUI.TransparencyValue or 0
    		-- Window.UIElements.Main.Background.ImageLabel.ImageTransparency = Value and Config.CUSTOMGUI.TransparencyValue or 0
    	...
    	function Window:GetUIScale(v)
    		return Config.CUSTOMGUI.UIScale
    	end
    
    	function Window:SetUIScale(v)
    		Config.CUSTOMGUI.UIScale = v
    		Tween(Config.CUSTOMGUI.UIScaleObj, 0.2, { Scale = v }, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
    		return Window
    	end
    	...
    	function Window:EnableMobileControls()
    		if not UserInputService.TouchEnabled then
    			return
    		end
    
    		for _, obj in pairs(Window.UIElements.Main:GetDescendants()) do
    			if not ((obj:IsA("TextButton") or obj:IsA("ImageButton")) and obj.AbsoluteSize.Y < 40 and not obj:FindFirstChild("TouchHitbox")) then
    				continue
    			end
    	...
    	local TabModuleMain = require("components/window/Tab")
    	local SectionModule = require("components/window/Section")
    	local TabModule = TabModuleMain.Init(Window, Config.CUSTOMGUI, Config.CUSTOMGUI.TooltipGui)
    	TabModule:OnChange(function(t)
    		Window.CurrentTab = t
    		if Window.ClearSearchBox then
    	...
    	function Window:Tab(TabConfig)
    		...
    		return TabModule.New(TabConfig, Config.CUSTOMGUI.UIScale)
    	end
    	...
    	local DialogModule = require("components/window/Dialog")
    	function Window:Dialog(DialogConfig)
    		...
    		local Dialog = DialogModule.Create(false, "Dialog", Window, Config.CUSTOMGUI, Window.UIElements.Main.Main)
    		...
    		local function CheckButtonsOverflow()
    			...
    			local totalWidth = ButtonsLayout.AbsoluteContentSize.X / Config.CUSTOMGUI.UIScale
    			local parentWidth = ButtonsContent.AbsoluteSize.X / Config.CUSTOMGUI.UIScale
    			...
    			if totalWidth > parentWidth then
    				...
    				if availableSpace > 0 then
    					...
    					for _, button in ipairs(Buttons) do
    						local buttonWidth = button.AbsoluteSize.X / Config.CUSTOMGUI.UIScale
    	...
    	local CurResizeInput = Config.CUSTOMGUI.GenerateGUID()
    	...
    	local function startResizing(input)
    		if Window.CanResize then
    			...
    			Tween(ResizeHandle.ImageLabel, 0.1, { ImageTransparency = 0.2 }):Play()
    
    			Creator.AddSignal(input.Changed, function()
    				if input.UserInputState == Enum.UserInputState.End then
    					if Config.CUSTOMGUI.CurrentInput and Config.CUSTOMGUI.CurrentInput ~= CurResizeInput then
    						return
    					end
    					...
    					Tween(ResizeHandle.ImageLabel, 0.17, { ImageTransparency = 0.6 }):Play()
    				end
    			end)
    		end
    	end
    	...
    	Creator.AddSignal(ResizeHandle.InputBegan, function(input)
    		...
    		then
    			...
    			Config.CUSTOMGUI.CurrentInput = CurResizeInput
    	...
    	if not Window.HideSearchBar then
    		local SearchContainer = New("Frame", {
    			Size = UDim2.new(1, -Window.UIPadding / 2, 0, 39),
    		...
    		local SearchBox = SearchContainer.Frame.SearchBox
    
    		local function applyFilter()
    			local query = SearchBox.Text:lower()
    			if Window.TabModule and Window.TabModule.SelectedTab then
    				local tab = Window.TabModule.Tabs[Window.TabModule.SelectedTab]
    				if tab then
    					for _, element in ipairs(tab.Elements) do
    						if element.__type == "Section" then
    							local sectionHasMatch = false
    							for _, child in ipairs(element.Elements) do
    								local titleMatches = child.Title and child.Title:lower():find(query, 1, true) ~= nil
    								local descMatches = child.Desc and child.Desc:lower():find(query, 1, true) ~= nil
    								local match = (query == "") or titleMatches or descMatches
    								if child.ElementFrame then
    									child.ElementFrame.Visible = match
    								end
    								if match then
    							...
    							if element.ElementFrame then
    								element.ElementFrame.Visible = (query == "") or sectionHasMatch
    							end
    							if sectionHasMatch and element.Opened == false then
    								element:Open(true)
    							end
    						else
    							local titleMatches = element.Title and element.Title:lower():find(query, 1, true) ~= nil
    							local descMatches = element.Desc and element.Desc:lower():find(query, 1, true) ~= nil
    							local match = (query == "") or titleMatches or descMatches
    							if element.ElementFrame then
    								element.ElementFrame.Visible = match
    							end
    						end
    					end
    				end
...
modules["components/window/Openbutton"] = function()
    local OpenButton = {}
...
modules["components/window/Section"] = function()
    local Section = {}
    ...
    local TabModule = require("components/window/Tab")
...
modules["components/window/Tab"] = function()
    local cloneref = (cloneref or clonereference or function(instance)
    	return instance
    end)
    ...
    local Window, CUSTOMGUI, UIScale
    ...
    function TabModule.Init(WindowTable, CUSTOMGUITable, ToolTipParent, TabHighlight)
    	Window = WindowTable
    	CUSTOMGUI = CUSTOMGUITable
...
modules["config/Init"] = function()
    ...
    function ConfigManager:Init(WindowTable)
        if not WindowTable.Folder then
            warn("[ CUSTOMGUI.ConfigManager ] Window.Folder is not specified.")
            return false
        end
        if RunService:IsStudio() or not writefile then
            warn("[ CUSTOMGUI.ConfigManager ] The config system doesn't work in the studio.")
            return false
        end
        
        Window = WindowTable
        ConfigManager.Folder = Window.Folder
        ConfigManager.Path = "CUSTOMGUI/" .. tostring(ConfigManager.Folder) .. "/config/"
    ...
    function ConfigManager:SetPath(customPath)
        if not customPath then
            warn("[ CUSTOMGUI.ConfigManager ] Custom path is not specified.")
            return false
        end
        ...
        if isfile(ConfigModule.Path) then
            ...
            if success and configData and configData.__autoload then
                ...
                task.spawn(function()
                    ...
                    if success then
                        if Window.Debug then print("[ CUSTOMGUI.ConfigManager ] AutoLoaded config: " .. configFilename) end
                    else
                        warn("[ CUSTOMGUI.ConfigManager ] Failed to AutoLoad config: " .. configFilename .. " - " .. tostring(result))
                    end
                end)
            end
        end
...
modules["elements/Code"] = function()
    local Creator = require("modules/Creator")
    local New = Creator.New
    ...
    function Element:New(Config)
        ...
        end, Config.CUSTOMGUI.UIScale, Code)
...
modules["elements/Colorpicker"] = function()
    ...
    function Element:Colorpicker(Config, Window, CUSTOMGUI, OnApply)
    	local Colorpicker = {
    		__type = "Colorpicker",
    		Title = Config.Title,
    		Desc = Config.Desc,
    	...
    	local ColorpickerFrame = ColorpickerModule.Create(nil, "Dialog", Window, CUSTOMGUI, Window.UIElements.Main.Main)
    ...
    function Element:New(Config)
    	...
    	Creator.AddSignal(Colorpicker.UIElements.Colorpicker.MouseButton1Click, function()
    		if CanCallback then
    			Element:Colorpicker(Colorpicker, Config.Window, Config.CUSTOMGUI, function(color, transparency)
...
modules["elements/Group"] = function()
    local Creator = require("modules/Creator")
    local New = Creator.New
...
modules["elements/HStack"] = function()
    local Creator = require("modules/Creator")
    local New = Creator.New
...
modules["elements/Image"] = function()
    local Creator = require("modules/Creator")
    local New = Creator.New
...
modules["elements/Init"] = function()
    return {
    	Elements = {
    		...
    		Divider = require("elements/Divider"),
    		Space = require("elements/Space"),
    		Image = require("elements/Image"),
    		Group = require("elements/Group"),
    		HStack = require("elements/HStack"),
    		VStack = require("elements/VStack"),
    		Viewport = require("elements/Viewport"),
    		--Video       = require("elements/Video"),
    	},
    	Load = function(tbl, Container, Elements, Window, CUSTOMGUI, OnElementCreateFunction, ElementsModule, UIScale, Tab)
    							...
    							if ConfigManager.Parser[data.__type] then
    								task.defer(function()
    									...
    									if success then
    										...
    										warn(
    											"[ CUSTOMGUI ] Failed to apply pending config for '"
    												.. config.Flag
    												.. "': "
    												.. tostring(err)
    										)
...
modules["elements/Section"] = function()
    local Creator = require("modules/Creator")
    local New = Creator.New
    local Tween = Creator.Tween
    ...
    function Element:New(Config)
    	...
    	ElementsModule.Load(Section, Main.Outline.Content, ElementsModule.Elements, Config.Window, Config.CUSTOMGUI, function()
...
modules["elements/Slider"] = function()
    ...
    function Element:New(Config)
    	...
    	local CurInput = Config.CUSTOMGUI.GenerateGUID()
    	...
    	Creator.AddSignal(Slider.UIElements.SliderContainer.InputBegan, function(input)
    		...
    		if
    			...
    			or input.UserInputType == Enum.UserInputType.Touch
    		then
    			if Config.CUSTOMGUI.CurrentInput and Config.CUSTOMGUI.CurrentInput ~= CurInput then
    				return
    			end
    			Config.CUSTOMGUI.CurrentInput = CurInput
...
modules["elements/Space"] = function()
    local Creator = require("modules/Creator")
    local New = Creator.New
...
modules["elements/Video"] = function()
...
modules["elements/Viewport"] = function()
    local cloneref = (cloneref or clonereference or function(instance)
    	return instance
    end)
...
modules["elements/VStack"] = function()
    local Creator = require("modules/Creator")
    local New = Creator.New
...
modules["modules/Creator"] = function()
    local cloneref = (cloneref or clonereference or function(instance)
    	return instance
    end)
    ...
    local DynamicShapeModule = require("modules/DynamicShape")
    ...
    function Creator.Init(CUSTOMGUITable)
    	CUSTOMGUI = CUSTOMGUITable
    
    	Creator.ThemeFallbacks = require("themes/Fallbacks")(Creator)
    
    	Creator.UIScale = CUSTOMGUITable.UIScale
    ...
    function Creator.SafeCallback(Function, ...)
    	...
    	if not Success then
    		if CUSTOMGUI and CUSTOMGUI.Window and CUSTOMGUI.Window.Debug then
    			local _, i = Event:find(":%d+: ")
    
    			warn("[ CUSTOMGUI: DEBUG Mode ] " .. Event)
    
    			return CUSTOMGUI:Notify({
    ...
    function Creator.Gradient(stops, props)
    	if CUSTOMGUI and CUSTOMGUI.Gradient then
    		return CUSTOMGUI:Gradient(stops, props)
    	end
    ...
    function Creator.Drag(mainFrame, dragFrames, ondrag)
    	...
    	for _, dragFrame in pairs(dragFrames) do
    		dragFrame.InputBegan:Connect(function(input)
    			...
    			then
    				if CUSTOMGUI and CUSTOMGUI.CurrentInput and CUSTOMGUI.CurrentInput ~= CurInput then
    					return
    				end
    				CUSTOMGUI.CurrentInput = CurInput
    ...
    function Creator.Image(Img, Name, Corner, Folder, Type, IsThemeTag, Themed, ThemeTagName)
    	...
    	elseif string.find(Img, "http") and not string.find(Img, "roblox.com") then
    		local FileName = "CUSTOMGUI/" .. Folder .. "/assets/." .. Type .. "-" .. Name .. ".png"
    		local success, response = pcall(function()
    			task.spawn(function()
    				...
    				if assetSuccess then
    					...
    					warn(
    						string.format(
    							"[ CUSTOMGUI.Creator ] Failed to load custom asset '%s': %s",
    							FileName,
    							tostring(asset)
    						)
    					)
    		...
    		if not success then
    			warn(
    				"[ CUSTOMGUI.Creator ]  '" .. identifyexecutor()
...
modules["modules/DynamicShape"] = function()
    local Creator
...
modules["modules/Highlighter"] = function()
...
modules["modules/Icons"] = function()
    local cloneref = (cloneref or clonereference or function(instance) return instance end)
...
modules["modules/Localization"] = function()
    local Localization = {}
...
modules["server/Discord.server"] = function()
...
modules["server/Icons.server"] = function()
...
modules["server/PullRequest.server"] = function()
    local HttpService = game:GetService("HttpService")
    local ReplicatedStorage = game:GetService("ReplicatedStorage")
...
modules["themes/Fallbacks"] = function()
...
modules["themes/Init"] = function()
    return function(CUSTOMGUI, Creator)
    	return {
    		Dark = {
    			Name = "Dark",
...
modules["utils/Acrylic/Blur"] = function()
    ...
    local viewportPointToWorld, getOffset = unpack(require("utils/Acrylic/Utils"))
...
modules["utils/Acrylic/Init"] = function()
    -- Credits: Fluent - Dawid
    ...
    local Acrylic = {
    	AcrylicBlur = require("utils/Acrylic/Blur"),
    	--CreateAcrylic = require("utils/Acrylic/"),
    	AcrylicPaint = require("utils/Acrylic/Paint"),
    }
...
modules["utils/Acrylic/Paint"] = function()
    -- Credits: Fluent - Dawid
    
    local Creator = require("modules/Creator")
    local AcrylicBlur = require("utils/Acrylic/Blur")
...
modules["utils/Acrylic/Utils"] = function()
    -- Credits: Fluent - Dawid
...
modules["utils/services/Init"] = function()
    return {
        platoboost = {
            Name = "Platoboost",
            ...
            New = require("utils/services/Platoboost").New
        },
        pandadevelopment = {
            ...
            New = require("utils/services/PandaDevelopment").New
        },
        luarmor = {
            Name = "Luarmor",
            ...
            New = require("utils/services/Luarmor").New
        },
        junkiedevelopment = {
            ...
            New = require("utils/services/JunkieDevelopment").New
        },
...
modules["utils/services/JunkieDevelopment"] = function()
...
modules["utils/services/Luarmor"] = function()
    --[[
...
modules["utils/services/PandaDevelopment"] = function()
    --[[
...
modules["utils/services/Platoboost"] = function()
...
modules["build/package"] = function()
    ...
    return [[
    {
        ...
        "version": "1.6.65",
        "main": "./dist/main.lua",
        "repository": "https://github.com/Footagesus/CUSTOMGUI",
